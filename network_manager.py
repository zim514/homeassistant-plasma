...
class NetworkManager:
    ...
    async def wait(self, mode):
        while not self.isconnected():
            await self._handle_status(mode, None)
            await uasyncio.sleep_ms(1000)

    async def _handle_status(self, mode, status):
        if callable(self._status_handler):
            await self._status_handler(self._ifname[mode], status, self.ifaddress())

    async def _handle_error(self, mode, msg):
        if callable(self._error_handler):
            if await self._error_handler(self._ifname[mode], msg):
                return
        raise RuntimeError(msg)

    async def client(self, ssid, psk):
        if self._sta_if.isconnected():
            await self._handle_status(network.STA_IF, True)
            return
        ...
        try:
            ...
            await self._handle_status(network.STA_IF, False)
            await self._handle_error(network.STA_IF, "WIFI Client Failed")

    async def access_point(self):
        if self._ap_if.isconnected():
            await self._handle_status(network.AP_IF, True)
            return
        ...
        try:
            ...
            self._sta_if.active(False)
            await self._handle_status(network.AP_IF, False)
            await self._handle_error(network.AP_IF, "WIFI Access Point Failed")
